...
function SonarRings({ active, phase }) {
  ...
  return (<div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
    {Array.from({ length: phase }, (_, i) => (
      <div key={i} style={{ position: "absolute", left: "50%", top: "50%", width: `${60+i*40}px`, height: `${60+i*40}px`, marginLeft: `${-(30+i*20)}px`, marginTop: `${-(30+i*20)}px`, borderRadius: "50%", border: `1.5px solid rgba(34,197,94,${0.5-i*0.05})`, animation: `sonarExpand 1.2s ease-out ${i*0.15}s`, opacity: Math.max(0.1, 0.6-i*0.06) }} />
    ))}
  </div>);
}
...
export default function BatLyfPublic() {
  ...
  const Landing = () => (
    <div style={{ position: "relative", width: "100%", minHeight: "100vh", background: "linear-gradient(180deg,#020810 0%,#06101d 30%,#0a1628 55%,#0c1a20 75%,#0f1f15 100%)", overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      {Array.from({ length: 120 }, (_, i) => ({ x: Math.random()*100, y: Math.random()*100, s: Math.random()*2.5+0.5, d: Math.random()*4, du: 2+Math.random()*3 })).map((s, i) => (
        <div key={i} style={{ position: "absolute", left: `${s.x}%`, top: `${s.y}%`, width: `${s.s}px`, height: `${s.s}px`, background: "#fff", borderRadius: "50%", animation: `twinkle ${s.du}s ease-in-out ${s.d}s infinite alternate`, boxShadow: s.s > 1.5 ? `0 0 ${s.s*3}px rgba(255,255,255,0.3)` : "none" }} />
      ))}

      {/* Moon */}
      <div style={{ position: "absolute", top: "8%", right: "15%", width: "120px", height: "120px", borderRadius: "50%", background: "radial-gradient(circle at 35% 35%, #f5f0d0, #d4c98a 60%, #a89860 100%)", boxShadow: "0 0 60px rgba(245,240,208,0.15), 0 0 120px rgba(245,240,208,0.08), inset -15px -10px 30px rgba(100,90,50,0.4)", animation: "moonGlow 6s ease-in-out infinite alternate", opacity: heroReady ? 1 : 0, transition: "opacity 2s ease 0.5s" }}>
      ...
      {/* Flying bats */}
      {[...Array(8)].map((_, i) => (
        <div key={i} style={{ position: "absolute", top: `${12+(i%4)*15+Math.sin(i)*8}%`, color: "#1a2a1f", width: `${22+(i%3)*10}px`, animation: `flyAcross ${14+i*3}s linear ${i*2.5}s infinite`, opacity: 0.3+(i%3)*0.15, zIndex: 1, filter: `blur(${i > 5 ? 1 : 0}px)` }}><FlyingBat /></div>
      ))}

      {/* Fireflies */}
      {[...Array(24)].map((_, i) => (
        <div key={`f${i}`} style={{ position: "absolute", left: `${10+Math.random()*80}%`, top: `${45+Math.random()*45}%`, width: "3px", height: "3px", borderRadius: "50%", background: i%3===0 ? "#86efac" : i%3===1 ? "#fde68a" : "#93c5fd", animation: `firefly ${4+Math.random()*5}s ease-in-out ${Math.random()*4}s infinite alternate`, zIndex: 3, boxShadow: `0 0 8px ${i%3===0 ? "rgba(134,239,172,0.6)" : i%3===1 ? "rgba(253,230,138,0.5)" : "rgba(147,197,253,0.4)"}` }} />
      ))}
      ...
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 24px", maxWidth: "800px", opacity: heroReady ? 1 : 0, transform: heroReady ? "translateY(0)" : "translateY(40px)", transition: "all 1.5s cubic-bezier(0.16,1,0.3,1) 0.8s" }}>
        ...
        <button onClick={() => setView("dashboard")} style={{ padding: "18px 52px", borderRadius: "16px", border: "1px solid rgba(34,197,94,0.3)", background: "linear-gradient(135deg,rgba(34,197,94,0.12),rgba(16,185,129,0.06))", color: "#86efac", fontSize: "18px", fontWeight: "700", cursor: "pointer", letterSpacing: "1px", backdropFilter: "blur(10px)", boxShadow: "0 0 30px rgba(34,197,94,0.1), inset 0 1px 0 rgba(255,255,255,0.09)", animation: "ctaPulse 3s ease-in-out infinite", transition: "all 0.3s" }}>
          Enter the Night →
        </button>
      ...
      <div style={{ position: "absolute", bottom: "30px", left: "50%", transform: "translateX(-50%)", zIndex: 10, textAlign: "center", animation: "scrollBounce 2.5s ease-in-out infinite", opacity: heroReady ? 0.35 : 0, transition: "opacity 1s ease 3s" }}>
  ...
  const Dashboard = () => {
    ...
    return (<div>
      <div style={{ position: "relative", borderRadius: "24px", overflow: "hidden", background: "linear-gradient(135deg,#06101d,#0c1a20,#0f1f15)", padding: "48px 40px", marginBottom: "32px", border: "1px solid rgba(34,197,94,0.06)" }}>
        {[...Array(25)].map((_, i) => <div key={i} style={{ position: "absolute", left: `${Math.random()*100}%`, top: `${Math.random()*100}%`, width: `${1+Math.random()}px`, height: `${1+Math.random()}px`, background: "#fff", borderRadius: "50%", opacity: 0.15+Math.random()*0.25, animation: `twinkle ${2+Math.random()*3}s ease-in-out ${Math.random()*3}s infinite alternate` }} />)}
        {[...Array(3)].map((_, i) => <div key={`hb${i}`} style={{ position: "absolute", color: "rgba(134,239,172,0.06)", width: `${20+i*6}px`, top: `${15+i*20}%`, right: `${8+i*12}%`, animation: `batFloat ${3+i}s ease-in-out ${i*0.6}s infinite alternate` }}><FlyingBat /></div>)}
  ...
  const Facts = () => (<div>
    ...
    {BAT_FACTS.filter(f => factFilter==="all"||f.category===factFilter).map((f,i) => (
      <div key={f.id} onClick={() => setFactsRead(p => new Set([...p,f.id]))} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "20px", marginBottom: "8px", cursor: "pointer", animation: `fadeSlideUp 0.4s ease ${i*0.04}s both` }}>
  ...
  const Profiles = () => (<div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: "12px" }}>
      {BAT_PROFILES.map((b,i) => (<div key={b.id} onClick={() => setSelBat(selBat===b.id?null:b.id)} style={{ background: selBat===b.id?"rgba(139,92,246,0.05)":"rgba(255,255,255,0.08)", border: `1px solid ${selBat===b.id?"rgba(139,92,246,0.15)":"rgba(255,255,255,0.08)"}`, borderRadius: "18px", padding: "22px", cursor: "pointer", transition: "all 0.3s", animation: `fadeSlideUp 0.4s ease ${i*0.07}s both` }}>
  ...
  const Media = () => { const ts = (a) => ({ padding: "9px 18px", borderRadius: "11px", cursor: "pointer", fontSize: "12px", fontWeight: a?"700":"400", border: `1px solid ${a?"rgba(236,72,153,0.25)":"rgba(255,255,255,0.08)"}`, background: a?"rgba(236,72,153,0.06)":"rgba(255,255,255,0.08)", color: a?"#f9a8d4":"#4b5563", transition: "all 0.2s" }); return (<div>
    ...
    {mediaTab==="livefeed" && <div>
      <div style={{ borderRadius: "20px", overflow: "hidden", border: "1px solid rgba(239,68,68,0.12)", marginBottom: "22px" }}>
        <div style={{ height: "280px", position: "relative", background: "radial-gradient(ellipse at center,#141428,#050510)" }}>
          {[...Array(12)].map((_,i) => <div key={i} style={{ position: "absolute", left: `${12+Math.sin(i*1.2)*30+20}%`, top: `${8+(i%5)*16}%`, fontSize: `${14+(i%3)*3}px`, opacity: 0.35, animation: `batFloat ${2+(i%4)}s ease-in-out ${i*0.2}s infinite alternate`, color: "#4a6a5a" }}>🦇</div>)}
          <div style={{ position: "absolute", top: "12px", left: "12px", display: "flex", alignItems: "center", gap: "6px", background: "rgba(0,0,0,0.6)", padding: "4px 10px", borderRadius: "7px" }}><div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#ef4444", animation: "pulse 1.5s infinite" }} /><span style={{ fontSize: "10px", color: "#fca5a5", fontWeight: "600" }}>LIVE</span></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "10px" }}>
        {VIDEO_ITEMS.map((v,i) => <div key={v.id} onClick={() => setPlayVid(playVid===v.id?null:v.id)} style={{ background: playVid===v.id?"rgba(236,72,153,0.04)":"rgba(255,255,255,0.08)", border: `1px solid ${playVid===v.id?"rgba(236,72,153,0.15)":"rgba(255,255,255,0.08)"}`, borderRadius: "14px", overflow: "hidden", cursor: "pointer", transition: "all 0.3s", animation: `fadeSlideUp 0.4s ease ${i*0.05}s both` }}>
    ...
    {mediaTab==="vr" && <div>
      <div style={{ borderRadius: "20px", overflow: "hidden", border: "1px solid rgba(139,92,246,0.12)", marginBottom: "18px" }}>
        <div style={{ height: "320px", position: "relative", background: `radial-gradient(ellipse at ${50+vrAngle*10}% 50%,#1e1435,#050508)`, transition: "background 0.5s" }}>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", gap: "18px", fontSize: `${24*vrZoom}px`, transition: "font-size 0.3s" }}>{VR_SCENES[vrAngle].els.map((e,i) => <span key={i} style={{ animation: `batFloat ${2.5+i*0.5}s ease-in-out infinite alternate`, opacity: 0.75 }}>{e}</span>)}</div>
    ...
    {mediaTab==="echo" && <div>
      <div style={{ background: "linear-gradient(135deg,#0a1a0a,#0a0a1a)", border: "1px solid rgba(34,197,94,0.08)", borderRadius: "20px", padding: "30px", textAlign: "center" }}>
        ...
        <button onClick={startEcho} disabled={echoOn} style={{ padding: "13px 32px", borderRadius: "14px", border: "2px solid rgba(34,197,94,0.25)", background: echoOn?"rgba(34,197,94,0.1)":"linear-gradient(135deg,rgba(34,197,94,0.08),rgba(16,185,129,0.04))", color: "#86efac", fontSize: "14px", fontWeight: "700", cursor: echoOn?"default":"pointer", opacity: echoOn?0.5:1, animation: echoOn?"none":"pulseGlow 2s infinite" }}>{echoOn?`Scanning ${echoPh}/8`:"🔊 Emit Pulse"}</button>
  ...
  return (
    <div onClick={(e) => { if (!e.target.closest('[data-dropdown]')) setOpenDrop(null); }} style={{ minHeight: "100vh", background: "#0c1220", color: "#e8e6e1", fontFamily: "'Segoe UI',system-ui,-apple-system,sans-serif" }}>
      {badgeNotif && <div style={{ position: "fixed", top: "76px", right: "20px", zIndex: 300, background: "linear-gradient(135deg,#0f1f15,#132218)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: "14px", padding: "14px 20px", display: "flex", alignItems: "center", gap: "10px", boxShadow: "0 16px 48px rgba(0,0,0,0.6)", animation: "slideInRight 0.4s cubic-bezier(0.16,1,0.3,1)" }}><span style={{ fontSize: "26px" }}>{badgeNotif.icon}</span><div><div style={{ fontSize: "9px", color: "#7a8494", letterSpacing: "1px" }}>BADGE EARNED</div><div style={{ fontSize: "13px", fontWeight: "700", color: "#86efac" }}>{badgeNotif.name}</div></div></div>}
      ...
      {view !== "landing" && <header style={{ background: "rgba(5,10,18,0.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.03)", position: "sticky", top: 0, zIndex: 100 }}>
            ...
            {navCategories.map((cat) => {
              ...
              return (
                <div key={cat.label} style={{ position: "relative" }} data-dropdown="true">
                  ...
                  {isOpen && (
                    <div style={{ position: "absolute", top: "100%", left: "0", marginTop: "4px", background: "rgba(10,15,25,0.95)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "6px", minWidth: "180px", boxShadow: "0 12px 40px rgba(0,0,0,0.5)", zIndex: 200, animation: "fadeSlideUp 0.2s ease both" }}>
                      {cat.items.map(item => (
        ...
        {mobileOpen && (
          <div className="mobile-menu" style={{ padding: "8px 20px 16px", borderTop: "1px solid rgba(255,255,255,0.03)", animation: "fadeSlideUp 0.25s ease both" }}>
            {navCategories.map((cat) => {
              if (!cat.items) {
      ...
      <style>{`
        ...
        @keyframes pulseGlow{0%,100%{box-shadow:0 0 8px rgba(34,197,94,.12)}50%{box-shadow:0 0 24px rgba(34,197,94,.3)}}
        .bwl{animation:flapL .4s ease-in-out infinite alternate;transform-origin:35px 20px}
        .bwr{animation:flapR .4s ease-in-out infinite alternate;transform-origin:45px 20px}
        @keyframes flapL{from{transform:rotate(0)}to{transform:rotate(-18deg)}}
        @keyframes flapR{from{transform:rotate(0)}to{transform:rotate(18deg)}}
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{display:none}
        button:hover{filter:brightness(1.15)}
        button{transition:all .2s}

        /* Dashboard cards — entrance once, pop animation on hover */
        .dash-card{
          animation:fadeSlideUp 0.5s ease var(--delay, 0s) both;
